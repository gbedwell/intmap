...
def check_genome_idx(idx_dir, idx_name, long_read):
    ...
    if not long_read:
        if len(idx_files) < 6:
            raise Exception("Given bowtie2 index does not exist.")
    else:
        mmi_files = [file for file in idx_files if file.endswith('mmi')]
        if not mmi_files:
            raise Exception("Given minimap2 index does not exist.")
...
def check_executable(path, ex_name):
    if os.path.isfile(path) and os.access(path, os.X_OK):
        if os.path.basename(path) == ex_name:
            ...
            raise Exception(f'Given executable does not appear to be {ex_name}.')
    ...
    if os.path.isdir(path):
        ...
        if os.path.isfile(path) and os.access(path, os.X_OK):
            ...
            raise Exception(f'The executable {ex_name} does not exist at the given location.')

def align_pe(bt2_path, sam_path, g_idx_dir, g_idx_name, 
             file1, file2, out_nm, min_frag_len, max_frag_len, nthr,
             v_idx_dir, v_idx_name, aln_seed_len, aln_seed_mismatch):
    
    g_idx = f'{g_idx_dir}/{g_idx_name}'
    ...
    bt2_cmd1 = (f'{bt2_path} --end-to-end -x {g_idx} -1 {nonv_R1} -2 {nonv_R2} -p {nthr} '
                f'-I {min_frag_len} -X {max_frag_len} -L {aln_seed_len} -N {aln_seed_mismatch} '
...
def align_se(bt2_path, sam_path, g_idx_dir, g_idx_name, 
    ...
    bt2_cmd1 = (f'{bt2_path} --end-to-end -x {g_idx} -U {nonv_R1} -p {nthr} '
                f'-I {min_frag_len} -X {max_frag_len} -L {aln_seed_len} -N {aln_seed_mismatch} '
                f'-D 20 -R 3 -i S,1,0.50 --sam-append-comment | ')
...
def align_lr(mm2_path, sam_path, g_idx_dir, g_idx_name, 
    ...
    v_aln_cmd = (f'{mm2_path} -Layx {lr_type} -t {nthr} --MD --secondary no {v_idx}.mmi {file1} | ')
    rm_v_cmd1 = f'{sam_path} view -@ {nthr} -h -f 4 -b - | '
    rm_v_cmd2 = f'{sam_path} collate -O -@ {nthr} - | '
    rm_v_cmd3 = f'{sam_path} fastq -@ {nthr} -T RX,OX -0 {nonv_R1} -n -'
    ...
    bt2_cmd1 = (f'{mm2_path} -Layx {lr_type} -t {nthr} --MD --sam-hit-only {g_idx}.mmi {nonv_R1} | ')
